...
public class MatlabFunktionen3 {
	 ...
	 * @return
	 */
	public static Object[] genFraqzurück(double[] c, int n) {
		...
		for (int i = 1; i < n; i = i + 2) {
			num[0] = num[0] * Math.pow((c[i] / (PREFERENZEN.parseDataTempEnd - PREFERENZEN.parseDataTempix5)), 2);
			den = Matlab.conv(new double[] { 1, c[i] / c[i + 1], Math.pow(c[i], 2) }, den);
		}
		if (even == true)
			den = Matlab.colon(den, 0, n);
	 ...
	 *
	 */
	public static class Target implements MultivariateFunction {
		double[] normt;
		double[] normy;
		int r;
		double[] coeff;
		...
		public double value(double[] c) {
			...
			StatusBar.clear();
			StatusBar.showStatus("Aktuelle Ordnung:   " + r + "      Aktueller Fehler:    " + s);
