...
public class Model extends Observable {
	...
	public void setParameter(int ordnungvon, int ordnungbis, int filter, int fensterbreite) {
		...
		System.out.println("Länge daten " + t0.length);
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				for (int i = startOrd; i <= endOrd; i++) {
					...
					for (int k = startOrd; k < endOrd; k++) {//k=0;
						if (td[k] < 0) {
							d[k] = 1;
				...
				for (int i = 0; i < coeffBesteOrdung.length; i++) {
					System.out.println("OmegaQKoeff" + coeffBesteOrdung[i]);
				}
				...
				StatusBar.clear();
				StatusBar.showStatus(Matlab.print("B", new double[] { num[0] }));
				StatusBar.showStatus(Matlab.print("A", den));
				Matlab.print("A", den);
				//-----------------------------
				...
				for (int i = 0, k = den.length; i < den.length; i++) {
					if (i < den.length - 1) {
						sbden.append(roundNumber(den[i], 2)).append("s^" + ((k--) - 1) + " + ");
					} else {
						sbden.append(roundNumber(den[i], 2)).append("");
					}
				}
				...
				for (int i = 0; i < imag.length; i++) {
					...
					imag[i] = nullstellenBestOrd[i].getImaginary()
							/ (PREFERENZEN.parseDataTempEnd - PREFERENZEN.parseDataTempix5);
				}
	...
	private void setPolesBestOrd(double[] real, double[] imag) {
		...
		poleBestOrd = true;
		Matlab.print("real", real);
		Matlab.print("imag", imag);
