...
//			double tx5 = tx[ix5];
//
//			while ((y[iy5] < 0.5 && y[iy5 + 1] >= 0.5) == false) {//Klammergesetzt
//				iy5 = iy5 + 1;
//			}
//
//			Object[] fp2 = findpeaks(Matlab.colon(y, iy5, y.length - 1));
...
//			int di = 0;
//			int dt = 0;
//			PREFERENZEN.parseDataTempEnd=1;
//			PREFERENZEN.parseDataTempix5=0;
//			return new Object[] { di, dt };
//		}
//
//	}
...
//		int iy5 = 1;
	...
	try {
		...
		PREFERENZEN.tStart=10;
		StatusBar.showStatus("Berechnen Button Drücken");
		return null;
	}	
...
//			double T = ty[iymm] - t[iy5];
			double T = ty[iymm] - ty[iy5];		// 04.06.2017, KS
...
//
//        if(fensterbreite %2 == 0){             // bei gerader fensterbreite entsteht ein Offset durch assymetrie
//              fensterbreite = fensterbreite -1;
//        }
//        
//        int fbkorr = 6;                              // für Gauss muss das Fenster grösser gewählt werden
//        int einauslaenge = fensterbreite/2;
//        double[] Resultat = new double[zeitvektor.length];
//        double einausfaktor[] = Matlab.ones(zeitvektor.length);
//        double sum = 0;
...
//        }
//        
//        for(int i = 0; i <= fenster.length / 2; i++ ){
//              sum = sum + fenster[i];
//        }
//        
//        for(int i = 1; i <= einauslaenge; i++){
//              einausfaktor[i - 1] = 1 / sum;
//              einausfaktor[zeitvektor.length - i] = 1 / sum;
//              sum = sum + fenster[i + fenster.length / 2];
//        }
//        
//        gefaltet = Matlab.conv(zeitvektor, fenster);
//        
//        for (int i = 0; i < zeitvektor.length; i++) {
//              Resultat[i] = gefaltet[i + (fensterbreite * fbkorr) / 2] * einausfaktor[i];
//        }
//        return Resultat;
// }
	//--------07.06.2017
...
//        int fbkorr = 6;                              // für Gauss muss das Fenster grösser gewählt werden
//
//        if(fensterbreite > zeitvektor.length/fbkorr){ // begrenzt die maximale Fensterbreite auf 1/6 des Eingangvektors -> Kompromisslösung
//              fensterbreite = zeitvektor.length / fbkorr;
//        }
//        int einauslaenge = fensterbreite/2;
//        double[] Resultat = new double[zeitvektor.length];
//        double einausfaktor[] = Matlab.ones(zeitvektor.length);
...
//        }
//        
//        for(int i = 0; i < einauslaenge*fbkorr; i++){
//              einausfaktor[i] = 1 / sum;
//              einausfaktor[zeitvektor.length - i -1] = 1 / sum;
//              sum = sum + fenster[i + fenster.length / 2 +1];
//        }
//        gefaltet = Matlab.conv(zeitvektor, fenster);
//        
//        for (int i = 0; i < zeitvektor.length; i++) {
...
// }
	public static double[] smoothing(double[] zeitvektor, int fensterbreite) {
		fensterbreite = (int) (zeitvektor.length/1000.0*fensterbreite);
        if(fensterbreite %2 == 0){             // bei gerader fensterbreite entsteht ein Offset durch assymetrie
              fensterbreite = fensterbreite -1;
        }
        int fbkorr = 10;                              // für Gauss muss das Fenster grösser gewählt werden
 ...
 */
	public static class Target implements MultivariateFunction {// Fehlerfunktion 
		...
		public double value(double[] c) {
			...
			StatusBar.clear();
			StatusBar.showStatus("Aktueller Fehler: "+s);
...
//				imag[i] = im[i * 2 + 1];	
			...
			for (int i = 0; i < w.length; i++) {
				q[i] = -w[i] / (2 * real[i]);			// 01.06.2017, KS
