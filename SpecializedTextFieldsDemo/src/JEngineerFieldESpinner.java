 ...
 * @since 30.05.2014
 */
public class JEngineerFieldESpinner extends JTextField implements FocusListener, ActionListener {
	...
	private static final String[] EXP_UNITS = { "e-15", "e-12", "e-9", "e-6", "e-3", "e0", "e3", "e6", "e9", "e12",
			"e15" };
	private JEngineerFieldESpinner txtField = this;
	 ...
	 *            Number of columns.
	 */
	public JEngineerFieldESpinner(DecimalFormat formatter, int col) {
		super(col);
	 ...
	 *            Number of columns.
	 */
	public JEngineerFieldESpinner(int col) {
		super(col);
		init();
	}
	 ...
	 *            Number of columns.
	 */
	public JEngineerFieldESpinner(int digits, int col) {
		super(col);
	 ...
	 */
	public JEngineerFieldESpinner(int digits, int col, int outputmode) {
		super(col);
		if (digits < 3 || digits > 16)
			throw new IllegalArgumentException();
		this.digits = digits;
	...
	public static String getUnitOfValue(double val) {
		...
		int engExp = (int) Math.floor(exp / 3.0) * 3;
		try {
			unit = JEngineerFieldESpinner.UNITS[5 + engExp / 3];
	...
	public void setMaxValue(double maxValue) {
		...
		zeroEnabled = true;
		setToolTipText("Wert \u2264 " + DoubletoStringENG(maxValue, 4, JEngineerFieldESpinner.UNIT));
	}
	...
	public void setMinValue(double minValue) {
		...
		zeroEnabled = true;
		setToolTipText("Wert \u2265 " + DoubletoStringENG(minValue, 4, JEngineerFieldESpinner.UNIT));
	}
	...
	public void setRange(double minValue, double maxValue) {
		...
		this.maxValue = maxValue;
		zeroEnabled = true;
		setToolTipText(DoubletoStringENG(minValue, 4, JEngineerFieldESpinner.UNIT) + " \u2264 Wert \u2264 "
				+ DoubletoStringENG(maxValue, 4, JEngineerFieldESpinner.UNIT));
	}
	...
	private void addPopupMenu(Component component) {
		...
		if (outputMode == JEngineerFieldESpinner.ENG)
			rbmenuOutputEng.setSelected(true);
		rbmenuOutputEng.addActionListener(this);
		rbmenuOutputEng.setActionCommand("" + JEngineerFieldESpinner.ENG);
		...
		if (outputMode == JEngineerFieldESpinner.UNIT)
			rbmenuOutputUnit.setSelected(true);
		rbmenuOutputUnit.addActionListener(this);
		rbmenuOutputUnit.setActionCommand("" + JEngineerFieldESpinner.UNIT);
		...
		if (outputMode == JEngineerFieldESpinner.FLOAT)
			rbmenuOutputFloat.setSelected(true);
		rbmenuOutputFloat.addActionListener(this);
		rbmenuOutputFloat.setActionCommand("" + JEngineerFieldESpinner.FLOAT);
	...
	private void init() {
		verifyer = (new InputVerifier() {
			...
			public boolean verify(JComponent input) {
				...
				if (v > maxValue || v < minValue || (v == 0 && !zeroEnabled)) {
					errorMsg();
					return false;
				} else {
					if (edited) {
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent e) {
				...
				try {
					...
					} else if (inputUNIT && Arrays.asList(JEngineerFieldESpinner.UNITS).contains("" + caracter)) {
	...
	private String unitCheck(String input) {
		for (int i = 0; i < JEngineerFieldESpinner.UNITS.length; i++) {
			input = input.trim().replace(JEngineerFieldESpinner.UNITS[i], JEngineerFieldESpinner.EXP_UNITS[i] + " ");
		}
		return input;
	}
